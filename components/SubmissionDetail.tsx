...
function buildWhatsAppUrl(phone: string, name: string) {
  // Strip everything except digits, convert Dutch 06 → 316
      ...
      <div className="sticky top-0 z-10 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button
            ...
            disabled={deleting}
            className="flex items-center gap-1.5 text-xs font-medium text-red-600 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-950 px-2.5 py-1.5 rounded-lg border border-red-200 dark:border-red-800 hover:border-red-300 transition-colors disabled:opacity-50"
          >
        ...
        <section className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* WhatsApp button — only when phone is available */}
              {submission.phone && (
                <a
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            {notesDirty && (
              <button
                ...
                className="text-xs font-medium bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1 rounded-lg transition-colors disabled:opacity-50"
              >
                {notesSaving ? 'Opslaan…' : 'Opslaan'}
              </button>
            )}
            {notesSaved && !notesDirty && (
              <span className="text-xs text-green-600 dark:text-green-400">✓ Opgeslagen</span>
            )}
          </div>
          <textarea
            value={notes}

...
import {
  ...
  statusBadge, STATUS_LABELS_AAN,
} from '@/types/aanvragen'
import type { PlanningStatus, Toegewezene } from '@/types/aanvragen'
...
export default function PlanningTab() {
  ...
  const load = useCallback(async () => {
    ...
    if (res.ok) {
      ...
      // Planning tab only shows accepted aanvragen
  ...
  const inputCls = 'rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-1.5 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-pink-400'
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50">
              {['Taaknaam', 'Voortgang', 'Wie', 'Int. deadline', 'Opleverdatum', 'Status', 'Aangevraagd door'].map(h => (
                <th key={h} className="text-left text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide px-4 py-3 whitespace-nowrap">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {lijst.map(a => (
              ...
              >
                {/* Taaknaam */}
                <td className="px-4 py-3 font-medium text-gray-900 dark:text-gray-100 whitespace-nowrap max-w-[200px] truncate">
                  {a.zaak}
                  <p className="text-[11px] text-gray-400 font-normal truncate">{a.beschrijving}</p>
                </td>
                ...
                <td className="px-4 py-3 whitespace-nowrap" onClick={e => e.stopPropagation()}>
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="w-12 rounded border border-gray-200 dark:border-gray-700 bg-transparent text-center text-xs text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-1 focus:ring-pink-400 px-1 py-0.5"
                    />
                ...
                <td className="px-4 py-3 whitespace-nowrap" onClick={e => e.stopPropagation()}>
                  <input
                    type="date"
                    value={a.interne_deadline ?? ''}
                    disabled={saving}
                    onChange={e => patch(a.id, { interne_deadline: e.target.value || null })}
                    className={inputCls + ' text-xs'}
                  />
                </td>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <InfoItem label="Opleverdatum"    value={selected.opleverdatum ? new Date(selected.opleverdatum).toLocaleDateString('nl-NL', { day: 'numeric', month: 'long', year: 'numeric' }) : '—'} />
                <InfoItem label="Interne deadline" value={selected.interne_deadline ? new Date(selected.interne_deadline).toLocaleDateString('nl-NL', { day: 'numeric', month: 'long', year: 'numeric' }) : '—'} />
                ...
                <div className="space-y-2 mb-3">
                  ...
                  {selected.notities.map(n => (
                    <div key={n.id} className="rounded-lg bg-gray-50 dark:bg-gray-800 px-3 py-2">
                      ...
                      <p className="text-[11px] text-gray-400 mt-1">{n.auteur} · {new Date(n.created_at).toLocaleDateString('nl-NL', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</p>
                    </div>
                  ))}
                </div>
